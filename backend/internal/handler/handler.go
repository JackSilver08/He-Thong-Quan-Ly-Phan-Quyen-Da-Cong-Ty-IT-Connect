...
func (h *Handler) Login(c *gin.Context) {
	...
	hash, u, err := h.Repo.FindLoginUser(c, req.Username)
	if err != nil || u.Status != "ACTIVE" || !auth.VerifyPassword(hash, req.Password) {
		c.JSON(http.StatusUnauthorized, gin.H{"error": "invalid credentials"})
		return
	}
	token, err := auth.IssueToken(h.Cfg.JWTSecret, u.ID, u.Username, u.Role, h.Cfg.JWTExpiresHours)
	if err != nil {
		c.JSON(500, gin.H{"error": "token issue failed"})
		return
	}
...
func (h *Handler) Me(c *gin.Context) {
	...
	_, u, err := h.Repo.FindLoginUser(c, cl.Username)
	if err != nil {
...
func (h *Handler) CreateCompany(c *gin.Context) {
	...
	if err != nil {
		c.JSON(409, gin.H{"error": "company may already exist"})
		return
	}
...
func (h *Handler) CreateUser(c *gin.Context) {
	...
	if err != nil {
		c.JSON(409, gin.H{"error": "user may already exist"})
		return
	}
	h.audit(c, "CREATE", "USER", &u.ID, map[string]any{"username": u.Username})
	c.JSON(201, u)
}
...
func (h *Handler) CreateProject(c *gin.Context) {
	...
	if err != nil {
		c.JSON(409, gin.H{"error": "project may already exist"})
		return
	}
...
func (h *Handler) SetPermission(c *gin.Context) {
	...
	case "X", "NONE", "DENY":
		x.Level = "NONE"
	default:
		c.JSON(400, gin.H{"error": "level must be R/W/X or READ/WRITE/NONE"})
		return
	}
...
func (h *Handler) Dashboard(c *gin.Context) {
	...
	_ = h.Repo.DB.QueryRow(c, `SELECT COUNT(*) FROM users WHERE deleted_at IS NULL`).Scan(&users)
	_ = h.Repo.DB.QueryRow(c, `SELECT COUNT(*) FROM companies WHERE deleted_at IS NULL`).Scan(&companies)
	_ = h.Repo.DB.QueryRow(c, `SELECT COUNT(*) FROM projects WHERE deleted_at IS NULL`).Scan(&projects)
	_ = h.Repo.DB.QueryRow(c, `SELECT COUNT(*) FROM users WHERE status='RESIGNED' AND deleted_at IS NULL`).Scan(&resigned)
	c.JSON(200, gin.H{"users": users, "companies": companies, "projects": projects, "resigned": resigned})
}

func (h *Handler) audit(c *gin.Context, action, entity string, id *string, details map[string]any) {
	if v, ok := c.Get("claims"); ok {
		cl := v.(*auth.Claims)
		_ = h.Repo.AddAudit(c, cl.UserID, action, entity, id, details)
	}
}
